  ...
  describe 'UIATarget' do
    ...
    describe 'UIATarget.device_orientation' do
      it 'should call Calabash uia command with correct parameters and return correct response' do
        ...
        @uia.command.must_equal('target.deviceOrientation()')
        res.must_equal(3)
      end
      ...
      describe 'UIATarget.deactivate_app_for_duration and alias methods' do
        it 'should call Calabash uia command with correct parameters and return correct response' do
          ...
          @uia.command.must_equal('var x = target.deactivateAppForDuration(5); var MAX_RETRY = 5, retries = 0; while (!x && retries < MAX_RETRY) { x = target.deactivateAppForDuration(5); retries += 1}; x')
          res.must_equal(true)

          res = UIATarget.deactivate_app_for_duration(10)
          # $uia_command.must_equal('target.deactivateAppForDuration(10)')
          @uia.command.must_equal('var x = target.deactivateAppForDuration(10); var MAX_RETRY = 5, retries = 0; while (!x && retries < MAX_RETRY) { x = target.deactivateAppForDuration(10); retries += 1}; x')
          res.must_equal(true)
      ...
      describe 'UIATarget.drag_from_to_for_duration and alias methods' do
        it 'should call Calabash uia command with correct parameters and return correct response' do
          ...
          @uia.command.must_equal('target.dragFromToForDuration({x:100, y:100}, {x:200, y:200}, 3)')

          UIATarget.drag(300, 300, 400, 400, 5)
          @uia.command.must_equal('target.dragFromToForDuration({x:300, y:300}, {x:400, y:400}, 5)')
        end
      end
      ...
      describe 'UIATarget.pinch_close_from_to_for_duration and alias methods' do
        it 'should call Calabash uia command with correct parameters and return correct response' do
          ...
          @uia.command.must_equal('target.pinchCloseFromToForDuration({x:100, y:100}, {x:200, y:200}, 3)')

          UIATarget.pinch_close(300, 300, 400, 400, 5)
          @uia.command.must_equal('target.pinchCloseFromToForDuration({x:300, y:300}, {x:400, y:400}, 5)')
        end
      end
      ...
      describe 'UIATarget.pinch_open_from_to_for_duration and alias methods' do
        it 'should call Calabash uia command with correct parameters and return correct response' do
          ...
          @uia.command.must_equal('target.pinchOpenFromToForDuration({x:100, y:100}, {x:200, y:200}, 3)')

          UIATarget.pinch_open(300, 300, 400, 400, 5)
          @uia.command.must_equal('target.pinchOpenFromToForDuration({x:300, y:300}, {x:400, y:400}, 5)')
        end
      end
      ...
      describe 'UIATarget.flick_from_to and alias methods' do
        it 'should call Calabash uia command with correct parameters and return correct response' do
          ...
          @uia.command.must_equal('target.flickFromTo({x:100, y:100}, {x:200, y:200})')

          UIATarget.flick(300, 300, 400, 400)
          @uia.command.must_equal('target.flickFromTo({x:300, y:300}, {x:400, y:400})')
        end
      end
    end
  end
